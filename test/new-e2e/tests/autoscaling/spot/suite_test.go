...
const (
	...
	spotMinOnDemandAnnotation     = "autoscaling.datadoghq.com/spot-min-on-demand-replicas"
...
// spotEnabled is the label set on Deployments to opt in to spot scheduling.
var spotEnabled = map[string]string{spotEnabledLabelKey: spotEnabledLabelValue}

// spotAnnotations builds the spot scheduling annotation map for a Deployment.
func spotAnnotations(percentage, minOnDemand int) map[string]string {
	return map[string]string{
		spotPercentageAnnotation:  strconv.Itoa(percentage),
		spotMinOnDemandAnnotation: strconv.Itoa(minOnDemand),
	}
}
...
func TestSpotSchedulingKind(t *testing.T) {
	...
	workerNodes := []kubeComp.KindWorkerNode{
		...
		},
		{
			...
			Taints: []kubeComp.Taint{{Key: "karpenter.sh/capacity-type", Value: "spot", Effect: "NoSchedule"}},
		},
	}
...
// expectPending asserts that exactly count pods are Pending.
func (s *spotSchedulingSuite) expectPending(c *assert.CollectT, pods []corev1.Pod, count int) {
	actual := 0
	for _, p := range pods {
	...
	require.Equal(c, count, actual, "expected %d pending pods", count)
}
...
func (s *spotSchedulingSuite) identifyNodes() {
	...
	for _, node := range nodes.Items {
		...
		case karpenterCapacityTypeOnDemand:
	...
	s.Require().NotEmpty(s.onDemandNode, "no node with %s=on-demand found; check WithKindWorkerNodes", karpenterCapacityTypeLabel)
}
	...
	s.Require().Eventually(func() bool {
		...
		for _, wh := range whList.Items {
			for _, webhook := range wh.Webhooks {
				if strings.Contains(webhook.Name, "spot") {
					return true
				}
			}
		}
