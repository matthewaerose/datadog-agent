...
func (p *EBPFProbe) getRawPacketMaps(writeInactiveBuffer bool) (rawPacketEventMap, routerMap *lib.Map, err error) {
	...
	active, err := GetActiveRawPacketMapNumber(p.Manager)
	if err != nil {
		return nil, nil, err
	}
	var name string
	if writeInactiveBuffer {
		name = GetRawPacketClassifierMapNameNotUsed(active)
	} else {
		name = GetRawPacketClassifierMapNameNotUsed(1 - active)
	}
...
func (p *EBPFProbe) swapRawPacketRouterSelValue(active uint32) error {
	fmt.Printf("swapping raw_packet_router_sel value from %d to %d\n", active, 1-active)
	selMap, _, err := p.Manager.GetMap("raw_packet_router_sel")
	if err != nil {
		return err
	}
	if selMap == nil {
		return errors.New("unable to find `raw_packet_router_sel` map")
	}
	// push the value to the sel map
	return selMap.Put(uint32(0), uint32(1-active))
}

func (p *EBPFProbe) setupRawPacketProgs(progSpecs []*lib.ProgramSpec, progKey uint32, maxProgs int, collectionPtr **lib.Collection, writeInactiveBuffer bool) error {
	...
	if len(progSpecs) > 0 {
		...
		_, routerMap, err := p.getRawPacketMaps(writeInactiveBuffer)
	...
	*collectionPtr = col
	...
	var progArrayName string
	if writeInactiveBuffer {
		progArrayName = GetRawPacketClassifierMapNameNotUsed(active)
	} else {
		progArrayName = GetRawPacketClassifierMapNameNotUsed(1 - active)
	}
	// setup tail calls
...
func (p *EBPFProbe) ApplyRuleSet(rs *rules.RuleSet) (*kfilters.FilterReport, bool, error) {
	...
	if p.probe.IsNetworkRawPacketEnabled() {
		...
		if active, err := GetActiveRawPacketMapNumber(p.Manager); err != nil {
